...
setwd('/Users/marceloppisuner/Documents/GitHub/Diputacions/Cáceres/Eco')
...
data_subset <- merged_data %>% filter(codente == "10000DD000")

# Create graphs for each Expenditure_Area for codente 10000DD000
...
for(var in unique_vars) {
  ...
  ggsave(filename = paste0("cac_e_plot_", str_extract(var, "\\d+$"), ".png"), plot = p, width = 8, height = 6)
}
...
merged_data_2digit <- bind_rows(datasets_2digit) %>% filter(codente == "10000DD000")
...
merged_data_3digit <- bind_rows(datasets_3digit) %>% filter(codente == "10000DD000")
