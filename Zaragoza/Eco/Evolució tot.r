...
setwd('/Users/marceloppisuner/Documents/GitHub/Diputacions/Zaragoza/Eco')
...
for(var in unique_vars) {
  ...
  ggsave(filename = paste0("zar_e_plot_", str_extract(var, "\\d+$"), ".png"), plot = p, width = 8, height = 6)
}
...
merged_data_2digit <- bind_rows(datasets_2digit) %>% filter(codente == "50000DD000")
...
merged_data_3digit <- bind_rows(datasets_3digit) %>% filter(codente == "50000DD000")
